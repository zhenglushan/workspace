...
class FenCiImage():
    ...
    def returnValues(self):
        '''
        返回值为 当前文档的 关键词、tags标签
        :param content: 要进行分词的内容
        :param dict_name: 默认为空，说明是普通新闻站，如果不为空，则是关键词站点
        :return:
        '''
        # 1、过滤所有 HTML 标签
        self.content = filter_all_html(self.content)  # 过滤所有 html 标签
        ...
        if self.dict_path:
            ...
            result = jieba.analyse.extract_tags(self.content, topK=10, withWeight=True, allowPOS=allow_POS)
            for item in result:
