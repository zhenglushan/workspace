...
#     fulldomain = 'http://www.qd5u.com/'
#     name = get_name(fulldomain)
#     table_name = "祛痘" + '_' + name  # 保存数据的表名
...
#     rules = [
#         Rule(
#             LinkExtractor(allow=("/tag/\d+_\d+.html$")), process_request='process_request', follow=is_follow),
#
#         Rule(
#             LinkExtractor(allow=("/\d+.html$"), allow_domains=(host,)), process_links='process_links',
...
#             if len(title) == 0:
#                 title = response.css("div#main.main div.article h1::text")
#                 if title:
#                     title = title.extract()[0]
#                     title = filter_title(title)
#
#             teamp_body = response.css("div#article-content.article-content")
#             if teamp_body:
...
#     execute(['scrapy', 'crawl', 'www_qd5u_com'])
