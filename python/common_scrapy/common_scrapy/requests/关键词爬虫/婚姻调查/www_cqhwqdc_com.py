...
#     fulldomain = 'http://www.cqhwqdc.com/'
...
#     rules = [
#         Rule(
#             LinkExtractor(allow="list.aspx\?c=\d+.*"), process_request='process_request', follow=is_follow),
#         Rule(
#             LinkExtractor(allow="info.aspx\?id=\d+$"), process_links='process_links', process_request='process_request',
...
#         if len(title) == 0:
#             title = response.css("div.title h3::text")
#             if title:
#                 title = title.extract()[0]
#                 title = filter_title(title)
#                 title = title.replace('重庆', '[$城市$]').replace('上海', '[$城市$]')
#
#         body = response.meta['c_body']
#         tbody = response.css("div.maincontent.clearfix")
#         if tbody:
#             tbody = tbody.extract()[0]
#             tbody = filter_common_html(tbody)  # 过滤常见的 HTML 标签和属性
#             tbody = tbody.replace('重庆', '[$城市$]').replace('上海', '[$城市$]')
#             body = body + tbody
#
#         if title and body:
...
#     execute(['scrapy', 'crawl', 'www_cqhwqdc_com'])
