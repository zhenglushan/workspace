...
#     fulldomain = 'http://www.lijun021.com/'
...
#     rules = [
#         Rule(
#             LinkExtractor(allow=("/\d+.html$")), process_links='process_links',
...
#             if len(title) == 0:
#                 title = response.css("div.rights h1::text")
#                 if title:
#                     title = title.extract()[0]
#                     title = filter_title(title)
#                     title = title.replace("上海", "[$城市$]").replace("北京", "[$城市$]")
#
#             qianzhui = 0
#             qianzhui_temp = re.finditer('<div class="nrs">', response.text, flags=re.I | re.S)
#             for qz in qianzhui_temp:
#                 end = qz.end()
#                 qianzhui = end
#             houzhui = response.text.rfind('<div class="postnavi">')
#             teamp_body = response.text[qianzhui:houzhui]
#
#             if teamp_body:
#                 teamp_body = teamp_body.replace('<div', '<p').replace('</div>', '</p>')
#
#                 teamp_body = filter_common_html(teamp_body)  # 过滤常见的 HTML 标签和属性
#
#                 teamp_body = teamp_body.replace("<u>", "").replace("</u>", "")
#                 teamp_body = teamp_body.replace("上海", "[$城市$]").replace("北京", "[$城市$]")
#
...
#     execute(['scrapy', 'crawl', 'www_lijun021_com'])
