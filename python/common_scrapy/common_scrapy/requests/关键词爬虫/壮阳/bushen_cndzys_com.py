...
#     fulldomain = 'http://bushen.cndzys.com/'
#     name = get_name(fulldomain)
#     table_name = "壮阳" + '_' + name  # 保存数据的表名
...
#     rules = [
#         Rule(
#             LinkExtractor(allow=("bushen.cndzys.com/list/\d+.html.*")), process_request='process_request'
#             , follow=is_follow),
#         Rule(
#             LinkExtractor(allow=("bushen.cndzys.com.*/content/\d+.html$")), process_links='process_links',
...
#             if len(title) == 0:
#                 title = response.css("article.main section h1::text")
#                 if title:
#                     title = title.extract()[0]
#                     title = filter_title(title)
#
#             qianzhui_temp = re.finditer('<article class="main">', response.text, flags=re.I | re.S)
#             for qz in qianzhui_temp:
#                 end = qz.end()
#                 qianzhui = end
#             houzhui = response.text.rfind('<div class="page">')
#             teamp_body = response.text[qianzhui:houzhui]
#
#             if teamp_body:
#                 re_sub = '<section>.*?</section>'
#                 teamp_body = re.sub(re_sub, "", teamp_body, flags=re.I | re.S)
#
#                 teamp_body = filter_common_html(teamp_body)  # 过滤常见的 HTML 标签和属性
#
#                 teamp_body = teamp_body.replace('<p><br/></p>', '')
#
...
#     execute(['scrapy', 'crawl', 'bushen_cndzys_com'])
