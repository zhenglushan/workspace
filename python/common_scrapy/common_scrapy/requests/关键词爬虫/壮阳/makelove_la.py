...
#     rules = [
#         Rule(
#             LinkExtractor(allow=("makelove.la/page/\d+$")), process_request='process_request', follow=is_follow),
#
#         Rule(
#             LinkExtractor(allow=("\-\d+.html$"), deny=('makelove.la/wp\-')), process_links='process_links',
...
#             if len(title) == 0:
#                 title = response.css("header.article-header h1.article-title a::text")
#                 if title:
#                     title = title.extract()[0]
#                     title = filter_title(title)
#
#             teamp_body = response.css("article.article-content")
#
#             if teamp_body:
#                 teamp_body = teamp_body.extract()
#                 teamp_body = "".join(teamp_body)
#
#                 re_sub = '<p class="post-copyright">.*'
...
#     execute(['scrapy', 'crawl', 'makelove_la'])
