...
#     start_urls = [
#         fulldomain,
#         "http://www.n-r-c.cn/guojixinwen/",
#         "http://www.n-r-c.cn/guonaxinwen/",
#         "http://www.n-r-c.cn/shehuixinwen/",
#         "http://www.n-r-c.cn/junshixinwen/",
#         "http://www.n-r-c.cn/kejixinwen/",
#         "http://www.n-r-c.cn/yulexinwen/",
#         "http://www.n-r-c.cn/caijingxinwen/",
#         "http://www.n-r-c.cn/fangchanxinwen/",
#     ]
#
#     rules = [
#         Rule(LinkExtractor(allow="/list.*.html$"), process_request='process_request', follow=is_follow),
#         Rule(LinkExtractor(allow="/\d+.html$"), process_links='process_links', process_request='process_request',
...
#         过滤重复网址
#         :param links: 此时 link 里面的 url 网址是补全了的
#         :return:
#         '''
#         new_links = []
#         for link in links:
#             url = link.url
#             # print("经过处理的 URL 地址为： " + url)
...
#         if len(title) == 0:
#             title = response.css("header.entry-header h1.post-title::text")
#             if title:
#                 title = title.extract()[0]
#                 title = filter_title(title)
#
#         body = response.meta['c_body']
#         tbody = response.css("div.entry div.entry-inner")
#         if tbody:
#             tbody = tbody.extract()
#             tbody = ''.join(tbody)
#             tbody = filter_common_html(tbody)  # 过滤常见的 HTML 标签和属性
#             tbody = tbody.replace('&amp;nbsp &amp;nbsp', '').replace('<p></p>', '')
#             body = body + tbody
#
#         if title and body:
...
#     execute(['scrapy', 'crawl', 'www_n_r_c_cn'])
