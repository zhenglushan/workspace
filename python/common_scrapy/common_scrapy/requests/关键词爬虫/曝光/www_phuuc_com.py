...
#                      fulldomain,
#                  ] + [
#                      'http://www.phuuc.com/index_%d.html' % p for p in range(2, 200)
#                  ]
#
...
#         con_urls = response.css("div.mip-content h2 a.mip-cont-title::attr(href)")
#         if con_urls:
...
#         #     result = response.css('div.pagination ul li.active + li mip-link::attr(href)')
#         #     if result:
...
#             if len(title) == 0:
#                 title = response.css("div.mip-detail-title h1::text")
#                 if title:
...
#             # 前后截取正文 开始
#             qianzhui = 0
#             qianzhui_temp = re.finditer('<div class="mip-detail-content">', response.text, flags=re.I | re.S)
...
#
#             if tbody:
#                 tbody = tbody.replace('<div class="mip-detail-content">', '').replace(
...
#     execute(['scrapy', 'crawl', 'www_phuuc_com'])
