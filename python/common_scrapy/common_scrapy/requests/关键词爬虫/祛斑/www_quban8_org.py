...
#     fulldomain = 'http://www.quban8.org/'
...
#     rules = [
#         Rule(
#             LinkExtractor(allow=("/[a-z]+/.*$", "/page/\d+/$")), process_request='process_request', follow=is_follow),
#
#         Rule(
#             LinkExtractor(allow=("www.quban8.org.*/\d+.html$", "www.quban8.org.*/\d+.html/$")),
...
#             if len(title) == 0:
#                 title = response.css("div.left div.article h1::text")
#                 if title:
#                     title = title.extract()[0]
#                     title = filter_title(title)
#
#             teamp_body = response.css("div.left div.article div#article-content.article-content p")
#             if not teamp_body:
#                 teamp_body = response.css("div.left div.article p")
#             if not teamp_body:
#                 teamp_body = response.css("div.left div.article div")
#             if teamp_body:
#                 teamp_body = teamp_body.extract()
#                 teamp_body = "".join(teamp_body)
#
#             if teamp_body:
#                 teamp_body = teamp_body.replace("<div", "<p").replace("</div>", "</p>").replace('<br />', '<p>')
#                 re_sub = "<p>本文链接.*"
...
#     execute(['scrapy', 'crawl', 'www_quban8_org'])
