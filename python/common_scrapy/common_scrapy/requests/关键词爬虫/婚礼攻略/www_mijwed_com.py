...
#     fulldomain = 'https://www.mijwed.com/'
#     name = get_name(fulldomain)
#     table_name = "婚礼攻略" + '_' + name  # 保存数据的表名
#     root_domain = get_root_domain(fulldomain)
#     host = get_host(fulldomain)
#     allowed_domains = [host]
#     start_urls = [
#         fulldomain,
#         'https://www.mijwed.com/baike/'
#     ]
#
#     rules = [
#         Rule(
#             LinkExtractor(allow=("/baike/[a-z]+/$", "/baike/[a-z]+/\d+.html$")), process_request='process_request',
#             process_links='process_other', follow=is_follow),
#
#         Rule(
#             LinkExtractor(allow=("/baike/\d+.html$")), process_links='process_links', process_request='process_request',
...
#
#         if len(title) == 0:
#             title = response.css("div.alticle-fl.fl h1.caption::text")
#             if title:
#                 title = title.extract()[0]
#                 title = filter_title(title)
#
#         tbody = response.css("div.main div.detail-text")
#         if tbody:
#             tbody = tbody.extract()
#             tbody = ''.join(tbody)
...
#     execute(['scrapy', 'crawl', 'www_mijwed_com'])
