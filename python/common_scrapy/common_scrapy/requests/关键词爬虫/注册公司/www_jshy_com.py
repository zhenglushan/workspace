...
#     fulldomain = 'http://www.jshy.com/'
#     name = get_name(fulldomain)
#     table_name = "注册公司" + '_' + name  # 保存数据的表名
...
#     rules = [
#         Rule(
#             LinkExtractor(allow=("/a_\d+$", "/a_\d+_\d+$", "/[a-z]+.html$")), process_request='process_request',
#             follow=is_follow),
#
#         Rule(
#             LinkExtractor(allow=("www.jshy.com.*/a_\d+.html$", "www.jshy.com.*/neizhan/\d+.html$")),
#             process_links='process_links', process_request='process_request', callback='parse_detail',
#             follow=is_follow),
#     ]
#
...
#
#         if len(title) == 0:
#             title = response.css("div.post-head h1.post-title::text")
#             if not title:
#                 title = response.css("div.clearfix h1.fleft::text")
#             if title:
#                 title = title.extract()[0]
#                 title = filter_title(title)
#                 title = title.replace("吉客", "[$城市$]")
#
#         teamp_body = response.css("div.post-content")
#         if not teamp_body:
#             qianzhui = 0
#             qianzhui_temp = re.finditer('<!-- Article -->', response.text, flags=re.I | re.S)
#             for qz in qianzhui_temp:
#                 end = qz.end()
#                 qianzhui = end
#             houzhui = response.text.rfind(
#                 '<div style=" width: width: 100%;;margin: 0 auto;line-height: 30px;margin-top: 5px;">')
...
#
#             teamp_body = teamp_body.replace("吉客", "[$城市$]")
#
...
#     execute(['scrapy', 'crawl', 'www_jshy_com'])
