...
#     fulldomain = 'http://www.kr-lancy.com/'
...
#
#     start_urls = [
#         'http://www.kr-lancy.com/news/',
#     ]
#
#     rules = [
#         Rule(LinkExtractor(allow="/news/list.*.html$"), process_request='process_request', follow=is_follow),
#         Rule(LinkExtractor(allow="/news/\d+-cn.html$"), process_links='process_links',
...
#         if len(title) == 0:
#             title = response.css("div#shownews.active h1.title::text")
#             if title:
#                 title = title.extract()[0]
#                 title = filter_title(title)
#
#         body = response.meta['c_body']
#         tbody = response.css("div#shownews.active div.editor")
#         if tbody:
#             tbody = tbody.extract()
#             tbody = ''.join(tbody)
#             re_sub = 'style=[\'\"].*?[\'\"]'
...
#     execute(['scrapy', 'crawl', 'www_kr_lancy_com'])
