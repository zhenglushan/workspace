...
#     fulldomain = 'http://www.chuntianxueyuan.com/'
#     name = get_name(fulldomain)
#     table_name = "半永久" + '_' + name  # 保存数据的表名
...
#
#     rules = [
#         Rule(LinkExtractor(allow="/page/\d+$"), process_request='process_request', follow=is_follow),
#         Rule(LinkExtractor(allow="/archives/\d+.html$"), process_links='process_links',
...
#                 title = title.replace('北京春甜', '韩式半永久培训学校')
#
#         body = response.meta['c_body']
#         tbody = response.css("div.entry-content")
#         if tbody:
#             tbody = tbody.extract()
#             tbody = ''.join(tbody)
#             re_sub = 'style=[\'\"].*?[\'\"]'
#             tbody = re.sub(re_sub, '', tbody, flags=re.I | re.S)
#             re_sub = '<div class="bdsharebuttonbox">.*'
#             tbody = re.sub(re_sub, '', tbody, flags=re.I | re.S)
#             tbody = filter_common_html(tbody)  # 过滤常见的 HTML 标签和属性
#             tbody = tbody.replace('bjctxy888', '').replace('010-51292349', '').replace('北京春甜', '韩式半永久培训学校')
#             body = body + tbody
#
#         if title and body:
...
#     execute(['scrapy', 'crawl', 'www_chuntianxueyuan_com'])
