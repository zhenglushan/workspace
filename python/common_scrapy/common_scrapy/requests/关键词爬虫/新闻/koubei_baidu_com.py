...
#     fulldomain = 'https://koubei.baidu.com/'
#     name = get_name(fulldomain)
#     table_name = "新闻" + '_' + name  # 保存数据的表名
#     root_domain = get_root_domain(fulldomain)
#
#     start_urls = [
#         "https://koubei.baidu.com/news/getnewslistajax?page=%d" % pn for pn in range(1, 30)
#     ]
#
...
#             resp = requests.get(self.fulldomain)
#             cookiejar = resp.cookies
#             _now = cookiejar._now
#             start = start + "&_now=" + str(_now)
#             headers = self.headers
#             headers.update({'Referer': start})
#             yield Request(url=start, headers=headers)
#
#     def parse(self, response):
#         pattern = '"link":"(\\\/\\\/koubei\.baidu\.com\\\/news\\\/detail\\\/\d+)",'
...
#         if len(title) == 0:
#             title = response.css("div.news-content-title span.detail-title::text")
#         if title:
...
#
#         tbody = response.css("div.main-content")
#
#         if tbody:
#             tbody = tbody.extract()[0]
#             tbody = tbody.replace('src="//', 'src="https://')  # 替换图片的 src 和 http
...
#     execute(['scrapy', 'crawl', 'koubei_baidu_com'])
