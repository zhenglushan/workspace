...
#     fulldomain = 'http://www.yiyaweb.com/'
...
#     start_urls = [
#         'http://www.yiyaweb.com/faq/list_9_1.html'
#     ]
#
#     rules = [
#         Rule(
#             LinkExtractor(allow=("/faq/list_\d+_\d+.html$")), process_request='process_request',
#             process_links='process_other', follow=is_follow),
#
#         Rule(
#             LinkExtractor(allow=("/faq/\d+/\d+.html$")), process_links='process_links',
...
#
#         if len(title) == 0:
#             title = response.css("div.productinfo h1::text")
#             if title:
...
#         # 前后截取正文 开始
#         qianzhui = 0
#         qianzhui_temp = re.finditer('<div class="line"></div>', response.text, flags=re.I | re.S)
#         for qz in qianzhui_temp:
#             end = qz.end()
#             qianzhui = end
#         houzhui = response.text.find('<script type="application/ld+json">', qianzhui)
#         tbody = response.text[qianzhui:houzhui]
#
#         if tbody:
#             tbody = filter_common_html(tbody)  # 过滤常见的 HTML 标签和属性
#             tbody = tbody.replace('<u>', '').replace('</u>', '').replace('&nbsp;', '')
#             tbody = re.sub('[ ]+', ' ', tbody, flags=re.I | re.S)
#             body = body + tbody
#
...
#     execute(['scrapy', 'crawl', 'www_yiyaweb_com'])
