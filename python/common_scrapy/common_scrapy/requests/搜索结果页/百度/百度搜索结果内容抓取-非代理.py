...
from scrapy_mongodb_for_search.my_tools.common import raplace_domains, pc_user_agent_arr, arr_size
...
def more_thread_caiji(coll_arr):
    # 循环每个集合进行数据采集
    for _ in range(100):
        random.shuffle(coll_arr)
        for coll_name in coll_arr:
            collcetion = database[coll_name]
            for i in range(50):
...
if __name__ == '__main__':
    ...
    '''
        对集合进行分段处理，同时采集多个集合的数据
    '''
    line_threads = []
    collection_names_arr = arr_size(collection_names, 50)
    for coll_arr in collection_names_arr:
        line_thread = Thread(target=more_thread_caiji, args=(coll_arr,))
        line_threads.append(line_thread)
    for lt in line_threads:
        lt.start()
        time.sleep(0.1)
