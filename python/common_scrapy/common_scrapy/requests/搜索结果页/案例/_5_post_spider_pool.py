...
'''
使用搜索结果的数据发布到 SingleAllWebs 蜘蛛池
'''
...
from scrapy_mongodb_for_search.settings import SQL_DATETIME_FORMAT, MYSQL_HOST, MYSQL_USER, MYSQL_PASSWORD, \
    MYSQL_CHARSET, \
    MYSQL_DBNAME_SEARCH
from scrapy_mongodb_for_search.my_tools.tools.fenci import FenCi
from scrapy_mongodb_for_search.my_tools.tools.commons import post_SingleAllWebs
...
class SingleAllWebsPublish():
    ...
    def __init__(self, keyword, webs):
        ...
        # 选择一定量的数据
        results = self.do_select(randint(2, 3))  # randint(10, 20) randint(4, 8)
        # 收集已经使用的内容 id 数组
        id_arr = []
        if results:
...
if __name__ == '__main__':
    ...
    for keyword in web_sites.keys():
        ...
        for _ in range(500):
            SingleAllWebsPublish(keyword, webs)
        # SingleAllWebsPublish(keyword, webs)
