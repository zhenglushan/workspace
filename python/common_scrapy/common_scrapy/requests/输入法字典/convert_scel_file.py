...
# @FileName   : ConvertSogouPinYin.py
...
def scel2txt(file_name):
    ...
    with open(file_name, 'rb') as f:
        data = f.read()
    name = byte2str(data[0x130:0x338])
    name = name.replace(' ', '+').replace(' ', '+')
    print("词库名：", name)  # .encode('GB18030')
    ...
    return getChinese(data[startChinese:]), name
