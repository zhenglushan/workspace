...
if __name__ == '__main__':
    ...
    应该在采集完成之后，再进行过滤
    """
    keyword_dir = "消费者权益日"  # 要采集的关键词目录名称
    filter_word = [
        ["消费者权益日"],  # 必须包含某个关键词
        ["图片", "素材", "模板", "海报", "设计"]  # 必须包含某个关键词
    ]
    keyword_arr = ["消费者权益日图片", "消费者权益日素材", "消费者权益日模板", "消费者权益日海报", "消费者权益日设计"]
    ...
    for i in range(1, level_number + 1):
        ...
        if lines:
            ...
            XiaoFeiZheQuanYiRi(lines, filter_word, save_kw_file_path)