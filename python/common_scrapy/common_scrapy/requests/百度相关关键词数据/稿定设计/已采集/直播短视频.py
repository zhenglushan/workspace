...
class ZhuTu:
    def __init__(self, keywords, filter_word, save_kw_file_path):
        ...
        for keyword in keywords:
            ...
            is_while = True
            while is_while:
                request_url = 'http://www.baidu.com/s?ie=UTF-8&wd=' + parse.quote(
                    keyword)
                            ...
                            for text in text_arr:
                                ...
                                for word in filter_word:
                                    if isinstance(word, str):
                                        ...
                                        w3 = word[3]
                                        w4 = word[4]
                                        if (w0 in text) and (w1 in text) and (
                                                w2 in text
                                        ) and (w3 in text) and (w4 in text):
                                            temp.add(text)

                        # 匹配右侧 'rsv_re_ename':'网站优化','rsv_re_uri' 格式的关键词
                        re_sub = "'rsv_re_ename':'(.*?)','rsv_re_uri'"
                        rsv_res = re.findall(re_sub,
                                             response.text,
                                             flags=re.I | re.S)
                        if rsv_res:
